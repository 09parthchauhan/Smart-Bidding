...
def user_signup(request):
            if request.method == 'POST':
                ...
                if(len(username)<3):
                    error.append(1)
                    messages.success(request,"Username Field must be greater than 3 character.")
                if(len(password)<5):
                    error.append(1)
                    messages.success(request,"Password Field must be greater than 5 character.")
                if(password != confirm):
                    error.append(1)
                    messages.success(request,"Password field and Confirm Password Field can be same")
                if(len(error)==0):
                    ...
                    usr.save()
                    profile_obj = UserDetail.objects.create(usr=usr, auth_token = str(uuid.uuid4))
                    profile_obj.save()
            ...
            if type == 'COD':
                ...
                if Orders.objects.filter(products=prod_id).exists():
                    ...
                    if bill_amount:
                        ...
                        return render(request,"buyer/payment.html",{'paytmdict':data_dict})
...
def response(request):
    if request.method == "POST":
        ...
        if verify:
            if data_dict['RESPMSG'] == 'Txn Success':
...
def submit_review(request):
    ...
    if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect(url)
def faq(request):
    return render(request,'buyer/faq.html',{'category':category.objects.all()})